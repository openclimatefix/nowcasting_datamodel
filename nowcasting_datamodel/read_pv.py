...
def get_pv_systems(session: Session, pv_systems_ids: Optional[List[int]] = None, provider: Optional[str] = "pvoutput.org") -> List[PVSystemSQL]:
    """
    Get all pv systems
...
def get_latest_pv_yield(session: Session, pv_systems: List[PVSystemSQL], append_to_pv_systems:bool = False) -> Union[List[PVYieldSQL],List[PVSystemSQL]]:
    """
    ...
    query = query.where(PVSystemSQL.id == PVYieldSQL.pv_system_id,)
    ...
    # order by 'created_utc' desc, so we get the latest one
    query = query.order_by(PVSystemSQL.id, desc(PVYieldSQL.datetime_utc), desc(PVYieldSQL.created_utc))
    ...
    if not append_to_pv_systems:
        ...
        pv_systems_with_no_pv_yields = [pv_system for pv_system in pv_systems if pv_system.id not in pv_systems_with_pv_yields_ids]
